...
import { ChatSession as Session, SessionStatus } from '../types/session';
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  addSession: (session) => set((state) => {
    ...
    const activeSessions = newSessions.filter(s => s.status === SessionStatus.ACTIVE);
    const waitingQueue = newSessions.filter(s => s.status === SessionStatus.WAITING);
  ...
  assignSession: (sessionId, agentId) => set((state) => {
    ...
    return {
      sessions: newSessions,
      activeSessions: newSessions.filter(s => s.status === SessionStatus.ACTIVE),
      waitingQueue: newSessions.filter(s => s.status === SessionStatus.WAITING)
    };
  }),
